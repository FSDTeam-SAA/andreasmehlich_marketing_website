...
const testimonials = [
  {
    ...
    initials: "JD",
    avatar: "from-slate-200 via-sky-200 to-blue-500",
  },
  {
    name: "Olivia Wilson",
    ...
    initials: "OW",
    avatar: "from-amber-200 via-rose-300 to-fuchsia-600",
  },
  {
    name: "Michael Brown",
    ...
    initials: "MB",
    avatar: "from-cyan-100 via-blue-300 to-slate-700",
  },
  {
    name: "Sarah Johnson",
...
function Testimonials() {
  return (
    <section className="relative isolate overflow-hidden bg-[#010616] px-4 py-16 text-white sm:px-6 lg:px-8 lg:py-20">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_50%_18%,rgba(37,99,235,0.18),transparent_28%),radial-gradient(circle_at_6%_30%,rgba(14,165,233,0.09),transparent_28%),radial-gradient(circle_at_94%_34%,rgba(139,92,246,0.10),transparent_30%)]" />
      <div className="absolute left-1/2 top-10 -z-10 h-36 w-64 -translate-x-1/2 rounded-full border border-blue-400/15 opacity-60" />

      <div className="container mx-auto">
        <div className="mx-auto max-w-3xl text-center">
          <div className="inline-flex items-center rounded-full border border-blue-400/20 bg-blue-500/10 px-5 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-blue-300 shadow-[0_0_24px_rgba(37,99,235,0.20)]">
            Testimonials
          </div>

          <h2 className="mt-7 text-4xl font-semibold leading-[1.12] text-white sm:text-5xl lg:text-[52px]">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {testimonials.map((item) => (
            <article
              key={item.name}
              className="flex min-h-[320px] flex-col rounded-2xl border border-blue-300/15 bg-[#061126]/70 p-6 shadow-[0_0_40px_rgba(37,99,235,0.10)] backdrop-blur-sm transition-colors hover:border-blue-300/30"
            >
          ...
          <div className="mt-7 grid grid-cols-2 items-center gap-5 sm:grid-cols-3 lg:grid-cols-6">
            {brands.map(({ name, icon: Icon }) => (
              ...
              >
                ...
                <span className="max-w-[92px] text-sm font-bold uppercase leading-4 tracking-wide">
                  {name}
                </span>
              </div>
            ))}
