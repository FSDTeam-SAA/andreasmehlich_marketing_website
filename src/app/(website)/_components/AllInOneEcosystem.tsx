...
import { 
  ArrowRight, Link2, Brain, ShieldCheck, RefreshCw, Headphones 
} from "lucide-react"
...
export default function AllInOneEcosystem() {
  ...
  const footerFeatures = [
    { title: "Fully Integrated", desc: "All tools work together seamlessly.", icon: Link2, color: "text-blue-400 bg-blue-500/10" },
    { title: "AI-Powered", desc: "Intelligent automation that saves you time.", icon: Brain, color: "text-purple-400 bg-purple-500/10" },
    { title: "Secure & Reliable", desc: "Your data is safe with enterprise-grade security.", icon: ShieldCheck, color: "text-emerald-400 bg-emerald-500/10" },
    { title: "Always Evolving", desc: "New features and updates to keep you ahead.", icon: RefreshCw, color: "text-sky-400 bg-sky-500/10" },
    { title: "Dedicated Support", desc: "We're here to help you succeed.", icon: Headphones, color: "text-pink-400 bg-pink-500/10" },
  ]

  return (
    <section className="relative w-full overflow-hidden bg-[#00091E] px-6 py-20 text-white md:px-12 lg:px-20 select-none">
        ...
        <div className="grid min-h-[640px] grid-cols-1 items-center gap-14 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="z-10 flex flex-col items-start space-y-6 lg:col-span-5">
            ...
            <p className="max-w-md text-base leading-relaxed text-zinc-400">
              KoraAI brings every tool your business needs into one intelligent ecosystem — so everything works together, effortlessly.
            </p>
            <div className="pt-2">
              <Link
                href="#"
        ...
        <div className="mt-10 rounded-2xl border border-blue-300/10 bg-[#020a21]/70 p-5 shadow-2xl shadow-black/30 backdrop-blur-sm">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-5 md:divide-x md:divide-zinc-900">
            {footerFeatures.map((feat, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-center rounded-xl p-3 text-center transition-colors hover:bg-white/[0.03] md:rounded-none">
                  <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-xl text-zinc-200 ${feat.color}`}>
